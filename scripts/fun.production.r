...
production = function(data, date = 'Date', temperature = 'Temp', dissoxy = 'ODO_mgL', odo_sat = 'ODO_sat', wind = 'WSpd', smooth.set = 0.05, site.long = -75.510, site.lat = 38.988, gage_height = 4.5, method = 'bilinear', PAR = 'TotPAR', GPP = "csv/GPP.csv", R = "csv/R.csv", final = "csv/Coursey_cleaned.csv", NEP = "csv/NEP.csv", smooth = FALSE, smooth.time = 'Week', writedata = FALSE){
  ...
  cat('Calculating Schmidt Number\n')
	...
	# *24 to convert to m d-1
	cat('Calculating Gas Exchange\n')
	if(method == 'bilinear'){
	...
	# Power function from the same as above
	#(0.168 + 0.228 * data[[wind]]^2.2) * (Schmidt_Num / 600)^-0.5)/100)

	if(smooth == TRUE){
	  ## Smooth DO data over a week
	  cat('Smoothing DO\n')
	  weeks = levels(as.factor(data[[smooth.time]]))
	  smooth.week = NULL
	  smooth.week = as.vector(smooth.week)
	  for (y in 1:length(weeks))
	  {
	    x = data %>% filter(data[[smooth.time]] == weeks[y])
	    z = lowess(x[[date]], x[[dissoxy]], smooth.set)
	    zy = z$y
	    smooth.week = append(smooth.week, zy)
	  }
	  data = data %>% mutate(smoothDO = smooth.week)
	  # Calculate Change in DO over Time for determining slope per time period
	  # If there is a gap in days replace the first number with a 0
	  data = data %>% mutate(DOdt = ifelse(as_date(data[[date]]) == lag(as_date(data[[date]])), smoothDO - lag(smoothDO), 0))
	  # From 
	  data = data %>% mutate(Gas_exchange = K_wind * (C_sat - smoothDO), correctedDO = smoothDO + Gas_exchange)
	}else{
	  data = data %>% mutate(DOdt = ifelse(as_date(data[[date]]) == lag(as_date(data[[date]])), data[[dissoxy]] - lag(data[[dissoxy]]), 0))
